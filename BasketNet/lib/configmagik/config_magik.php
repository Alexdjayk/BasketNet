...
/**
 * Fonction qui va retourner un élément du fichier de configuration en fonction de la section voulue (ou pas)
 * 
 * @param 	varchar $key 		Nom de la clée à récupérer
 * @param 	varchar $section 	Nom de la section à récupérer
 * @return 	mixed null si pas de valeur, sinon retourne la valeur voulue
 * @access 	public
 * @author 	koéZionCMS
 ...
 */
	function get($key = null, $section = null) {
		...
		//get requested value
		//FIX : on va tester si la valeur à retourner existe bien dans la liste des VARS
		if($this->PROCESS_SECTIONS) { $value = isset($this->VARS[$section][$key]) ? $this->VARS[$section][$key] : false; }
		else { $value = isset($this->VARS[$key]) ? $this->VARS[$key] : false; }
	...
	*/
	function save( $path=null){
		...
		// config-header
		$content .= "; FICHIER GENERE AUTOMATIQUEMENT PAR KOEZION CMS\n";
		$content .= "; NE MODIFIEZ CE FICHIER QUE SI VOUS ETES SUR DE VOUS!!!\n";
...
/**
 * Fonction qui va retourner la liste des clés (avec les valeurs associées) pour une section données (ou pas)
 * 
 * @param 	varchar $section Nom de la section à récupérer
 ...
 * @author 	koéZionCMS
 * @version 0.1 - 02/03/2012 by FI
 * @version 0.2 - 05/03/2012 by FI - Mise en place de la récupération des clés par une sous chaine
 */
	function keys_values($section = null, $substr = 0) {
		
		//On contrôle qu'une section soit passée
		if($section !== null) {
			
			//On contrôle que cette section existe
			$sections = $this->listSections();
			
			$list = array();
			
			if(count($sections) > 0) {
				//Si on a demandé une recherche par sous-chaine
				if($substr) {
					foreach($sections as $k => $v) { //Parcours des sections 
						
						if(!substr_count($v, $section)) { //Si la sous-chaine ne correspond pas
							
							$err = "ConfigMagik::keys_values() - Section('$section') could not be found.";
							array_push($this->ERRORS, $err);
							return false;
						} else { //Si elle correspond on la stocke
				...
				//On va lister toutes les clées de la section voulue
				
				$all = array_keys($this->VARS[$localSection]);
				
				//Parcours de tous les éléments de la section
				foreach($all as $possible_key) {
					
					//Si l'élément parcouru existe et que ce n'est pas un tableau
					if(isset($this->VARS[$localSection][$possible_key]) && !is_array($this->VARS[$localSection][$possible_key])) {
											
						$list[$possible_key] = $this->VARS[$localSection][$possible_key]; //On le rajoute à la liste
					}
				}				
			}
			
			return $list; //Et on retourne cette liste
			
		//Si pas de section on retourne toutes les données
